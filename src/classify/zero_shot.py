...
OUTPUT_PATH = REPO_ROOT / "data" / "processed" / "corpus_labeled_zs.csv"

MODEL_NAME = "MoritzLaurer/DeBERTa-v3-large-mnli-fever-anli-ling-wanli"
HYPOTHESIS_TEMPLATE = "This paper is {}."

# Maps the short label (same names as the rule-based classifier) to the
# expanded phrasing actually fed to the NLI model. Tune the expanded
# phrases -- not the short names -- to shift accuracy on ambiguous
# classes; the short names are just for the output column.
LABELS: dict[str, str] = {
    "Empirical Study": (
...
EXPANDED_TO_SHORT = {v: k for k, v in LABELS.items()}
...
def build_pipeline():
    ...
    # NLI models have a 512-token cap; long abstracts must be truncated
...
def classify_papers(df: pd.DataFrame, clf=None) -> pd.DataFrame:
    ...
    three new columns: predicted_type_zs, confidence_zs, zs_scores.
    Pass an existing pipeline via `clf` to avoid reloading the model.
    """
    if clf is None:
        clf = build_pipeline()
    ...
    for idx, row in df.iterrows():
        ...
        # result["labels"] / result["scores"] arrive sorted descending.
